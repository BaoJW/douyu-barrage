...
func main() {
	...
	live := &douyulive.Live{
		...
		GiftComboSend: func(roomID int, m *douyulive.ComboSendModel) {
			log.Printf("【礼物连击】%v 赠送 %v 连击 %v 次", m.UserName, m.GiftName, m.ComboNum)
		},
		GiftComboEnd: func(roomID int, m *douyulive.ComboEndModel) {
			log.Printf("【连击结束】%v 赠送 %v(价值%v) 总共连击 %v 次", m.UserName, m.GiftName, m.Price, m.ComboNum)
		},
		GuardBuy: func(roomID int, m *douyulive.GuardBuyModel) {
			log.Printf("【用户上船】欢迎 %v - %v(%v) 上船", m.GiftName, m.UserName, m.UserID)
		},
		FansUpdate: func(roomID int, m *douyulive.FansUpdateModel) {
			log.Printf("【粉丝更新】当前粉丝数 %d", m.Fans)
		},
		SpecialGift: func(roomID int, m *douyulive.SpecialGiftModel) {
		...
		SuperChatMessage: func(roomID int, m *douyulive.SuperChatMessageModel) {
